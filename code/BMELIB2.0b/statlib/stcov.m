function [C,np]=stcovgui(Zi,cMSi,tMEi,Zj,cMSj,tMEj,rLag,rLagTol,tLag,tLagTol,dist,method);
  ...
  if nMSpairs==0,
    ...
    for it=1:nt,
      ...
      if nMEpairs==0
        ...
        % add by Hwa-Lung 2006/06/20 to avoid the problem of "out of
        % memory"
        
        if length(iMSmat_r)>50000
          idxr=unidrnd(length(iMSmat_r),50000,1);
          iMSmat_r=iMSmat_r(idxr);
          jMSmat_r=jMSmat_r(idxr);
        end

        if length(iMEmat_t)>50000
          idxt=unidrnd(length(iMEmat_t),50000,1);
          iMEmat_t=iMEmat_t(idxt);
          jMEmat_t=jMEmat_t(idxt);
        end          
        ...
        clear  Xhead Xtail idxValid headMean tailMean;
      end      
    end
    clear iMSmat_r jMSmat_r iMEmat_t jMEmat_t;
  end
end
