...
elseif ischar(harddata)
  if length(harddata)>=4 & same(harddata(end-3:end),'.mat')
    load(harddata);
    if ~exist('ph','var') | ~exist('zh','var')
      error(sprintf('The file %s must contain the variables ph and zh',harddata));
    end
  elseif length(harddata)>=4 & same(harddata(end-3:end),'.dat')
...
elseif ischar(softdata)
  if length(softdata)>=4 & same(softdata(end-3:end),'.mat')
    load(softdata);
  ...
  elseif length(softdata)>=4 & same(softdata(end-3:end),'.dat')
