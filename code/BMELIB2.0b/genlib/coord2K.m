...
% 20120529-AK: If model cell has more than one components, then model{i}
% is a cell, too. This messes up things, because later eval does not accept 
% cells in its arguments. Simplify model{i} to be a char array in all cases.
for k=1:nm
  if (iscell(model{k}))
    model{k} = model{k}{1};
  end
