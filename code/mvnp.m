function   [P,Err,Info]=mvnp(lowerL,upperL,covMatrix,maxpts,aEps,rEps)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  N = 0;
  Varsum = 0;
  d(1) = phi(lowerL(1)/C(1,1));
  e(1) = phi(upperL(1)/C(1,1));
  f(1) = e(1) -d(1);
  ...
  % b)
    ...
    for(i = 2:m)
      y(i-1) = 1/(phi(d(i-1)+w(i-1)*(e(i-1)-d(i-1))));
      %aux = 0;
      %for(j = 1:(i-1))
      %  aux = aux + C(i,j)*y(j);
      %end
      aux = C(i,:)*y(:);
      d(i) = phi((lowerL(i)-aux(i-1))/C(i,i));
      e(i) = phi((upperL(i)-aux(i-1))/C(i,i));
      f(i) = (e(i)-d(i))*f(i-1);
    end
